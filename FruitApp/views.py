...
from .apps import FruitappConfig
...
def fetch_calories(prediction):
    try:
        ...
        print(e)
        return HttpResponse(json.dumps({'error':'Service timeout'}))
...
def processed_img(img_path):
    ...
    answer = FruitappConfig.model.predict(img)
...
def ImageUploadTempView(request):
    try:
        if request.method == 'POST':
            ...
            if imageFile:
                save_image_path = './media/' + imageFile.name
                with open(save_image_path, "wb") as f:
                    f.write(imageFile.file.read())
                if imageFile is not None:
                    result = processed_img(save_image_path)
                    print("Result*******", result)
                    context = {"status":"Successful", "predicate":result}
                    if result in vegetables:
                        context.update({'category': 'Vegetables'})
                    else:
                        context.update({'category': 'Fruit'})
                    cal = fetch_calories(result)
                    if cal:
                        context.update({'cal': f"{cal} (100 grams)"})
